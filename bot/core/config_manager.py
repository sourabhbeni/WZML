...
try:
    ...
    # wz_bin is only shipped inside the official docker image; fall back to the
    # last known binary names so the bot also starts on plain pip installs.
    _BIN_NAMES = (
        "blitzfetcher",  # aria2
        "stormtorrent",  # qbittorrent
        "mediaforge",  # ffmpeg
        "ghostdrive",  # rclone
        "newsripper",  # sabnzbd
    )
